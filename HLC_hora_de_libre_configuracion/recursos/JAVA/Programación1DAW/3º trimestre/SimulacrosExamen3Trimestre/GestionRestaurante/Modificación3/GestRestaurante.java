package GestionRestaurante.Modificación3;
...
public class GestRestaurante {
    ...
    // Metodo para buscar la letra del DNI
    public void buscarLetraDni() {
        String ruta = "pedidos";
        char letraDni = 'w';
        ...
        while (true) {
            try (BufferedReader br = new BufferedReader(new FileReader(ruta + ".txt"))) {
                String linea;
                char letraDniMinusculas = Character.toLowerCase(letraDni);
                int contador = 0;
                
                while ((linea = br.readLine()) != null) {
                    String lineaMinusculas = linea.toLowerCase();

                    for (char caracter : lineaMinusculas.toCharArray()) {
                        char caracterMinuscula = Character.toLowerCase(caracter);

                        if (caracterMinuscula == letraDniMinusculas) {
                            contador++;
                        }
                    }
                }
                ...
                if (contador != 0) {
                    System.out.println("La letra de tu dni (" + letraDni + ") aparece: " + contador);
                    break;
                } else {
                    if (letraDni == 'z' || letraDni == 'Z') {
                        letraDni = 'A';
                    } else {
                        letraDni = (char)(letraDni + 1);
                    }
                }
            } catch (IOException e) {
                System.out.println(e.getMessage());
