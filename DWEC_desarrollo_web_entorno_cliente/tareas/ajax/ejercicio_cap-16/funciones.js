...
function inicializarEventos() {
  ...
  for (const element of vec) {
    element.addEventListener("mouseover", mostrarToolTip, false);
    element.addEventListener("mouseout", ocultarToolTip, false);
    element.addEventListener("mousemove", actualizarToolTip, false);
  }
...
function mostrarToolTip(e) {
  ...
  var ref;
  ref = e.target;
  recuperarServidorTooltip(ref.getAttribute("class"));
}
...
function recuperarServidorTooltip(codigo) {
  ...
  conexion1.open("GET", `cargar_anatomia.php?id=${codigo}`, true);
  conexion1.send();
}
