...
public class GestRestaurante {
    ...
    public void agregarPedido() {
        ...
        Pedido pedido = new Pedido(numeroMesa, pedidioPlatos, precioCuenta);
        pedidos.add(pedido);
    ...
    public void cargarPedidos() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(ruta + ".txt"))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                double precioCuenta = Double.parseDouble((datos[datos.length - 1]));
    ...
    public void guardarPlatos() {
        String ruta = "platos";
        ArrayList<ListaProducto> productosLista = new ArrayList<>();

        // Añado los productos a la nueva lista de platos para trabajar con ella después
        for (Pedido pedido : pedidos) {
            for (Plato plato : pedido.getLitaPlatos()) {
                productosLista.add(new ListaProducto(plato.getNombre(), 1, plato.getPrecioPlato()));
            }
        }

        // Nuevo arraylist donde estarán los productos sin repetirse
        ArrayList<ListaProducto> productosCombinados = new ArrayList<>();

        // Bucle for que se encarga de añadir los porductos (en caso de que se repitan
        // solo suma sus variables)
        for (ListaProducto producto : productosLista) {
            boolean encontrado = false;
            for (ListaProducto combinado : productosCombinados) {
                if (producto.getNombre().equalsIgnoreCase(combinado.getNombre())) {
                    combinado.setExistencia(combinado.getExistencia() + producto.getExistencia());
                    combinado.setPrecioTotal(combinado.getPrecioTotal() + producto.getPrecioTotal());
                    encontrado = true;
                    break;
                }
            }
        ...
        // Try que se encarga de escribir el archivo
        try (PrintWriter out = new PrintWriter(new FileWriter(ruta + ".txt"))) {
            for (ListaProducto listaProducto : productosCombinados) {
                out.println(listaProducto.getNombre() + "," + listaProducto.getExistencia() + ","
                        + listaProducto.getPrecioTotal() + "pts.");
            }
        } catch (IOException e) {
            System.out.println(e.getMessage());
        }
