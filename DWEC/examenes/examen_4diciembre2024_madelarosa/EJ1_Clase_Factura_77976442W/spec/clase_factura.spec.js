describe("Testeo de la clase Factura", () => {
  ...
  let articulos1 = [
    ...
    {
      codigo: "A730",
      descripcion: "Termo eléctrico 80 litros",
      cantidad: 1,
      precio: 152.22,
    },
    {
      codigo: "A735",
      descripcion: "Termo eléctrico 40 litros",
      cantidad: 2,
      precio: 88.12,
    },
  ];
  let totalFactura1 = 0;
  articulos1.forEach((item) => {
    totalFactura1 += item.cantidad * item.precio;
  });
  let palabraBuscar1 = "grifo";
  let busqueda1 = articulos1
    .filter((item) => item.descripcion.toLowerCase().includes(palabraBuscar1))
    .map((item) => item.codigo);
  let palabraBuscar2 = "eléctrico";
  let busqueda2 = articulos1
    .filter((item) => item.descripcion.toLowerCase().includes(palabraBuscar2))
    .map((item) => item.codigo);
  ...
  it(`El getter letraDNICLiente debería devolver ${cliente1.dni.at(
    -1
  )}`, () => {
